...
function getEmptyReview(fullname = '', readAt = '', rating = '', txt = '') {
  return {
    id: '',
    fullname,
    readAt,
...
function getReadingLevel(book) {
  console.log('book: ', book)
  let readingLevel = ''
  if (book.pageCount > 200) {
    readingLevel = (book.pageCount > 500) ? 'Serious Reading' : 'Decent Reading'
...
function checkVintage(book) {
  const oneYear = 31536000000  //in milliseconds
  const tenYears = 10 * oneYear
  const now = new Date()
  const publishedDate = new Date(book.publishedDate)
  const timePassed = now - publishedDate
  if (timePassed < oneYear) return 'new'
  if (timePassed > tenYears) return 'vintage'
}

function checkPriceRange(book) {
...
function _createBooks() {
  ...
  if (!books || !books.length) {
    ...
    bookData.forEach(book => book['reviews'] = [])
    books = bookData
    console.log('books: ', books[0].listPrice)
    utilService.saveToStorage(BOOK_KEY, books)
  }
}

function _createBook(title, listPrice, subtitle, authors, publishedDate, description, pageCount, categories, thumbnail, language) {
